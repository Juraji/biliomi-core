...
class UsersService(
    ...
    fun createUser(username: String, password: String): Mono<UserPrincipal> =
        validateAsync {
            ...
            .validateAsync {
                succeeds(commandGateway.send<String>(CreateUserCommand(it.userId)))
            }

    fun deleteUser(userId: String): Mono<UserPrincipal> = userPrincipalRepository
        .deleteById(userId)
        .validateAsync {
            succeeds(commandGateway.send<String>(DeleteUserCommand(it.userId)))
        }
    ...
    fun updatePassword(userId: String, currentPassword: String, newPassword: String): Mono<UserPrincipal> =
